...
mod tests {
  use crate::{
    dynamiccolor::{DynamicSchemeBuilder, MaterialDynamicColors, Variant},
    utils,
  };
  ...
  fn dynamic_color_api() {
    ...
    let colors: Vec<DynamicColor> = COLORS.all_dynamic_colors();
    for color in colors.iter() {
      let argb = color.get_argb(&scheme);
      println!(
        "{}: #{:0>6x}",
        color.name,
        if utils::color::alpha_from_argb(argb) == 0xff {
          argb & 0x00ffffff
        } else {
          argb
        }
    ...
    // for color in COLORS.all_dynamic_colors() {
    //   println!("{}: {:x}", color.name(), color.get_argb(&scheme));
    // }
  }
}
